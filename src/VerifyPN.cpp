    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "-z") == 0)
        {
            ...
            printf("Usage: verifypn [options] model-file query-file\n"
                    ...
                    "                                       - 4  two alternating user defined reduction sequences."
                    "                                            the first is run to exhaustion, then the second is run to exhaustion, then the first again etc.\n"
                    "                                            eg -r 4 0,1,2,3 4,5 uses ((A,B,C,D)*(E,F)*)*"
